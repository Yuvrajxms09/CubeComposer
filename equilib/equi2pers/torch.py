...
def convert_grid(
    ...
    if method == "robust":
        ui = (theta - torch.pi) * w_equi / (2 * torch.pi)
        uj = (phi - torch.pi / 2) * h_equi / torch.pi
        ui += 0.5
        uj += 0.5
        ui %= w_equi
        uj %= h_equi
...
def run(
    ...
    # Compute mask
    mask = torch.zeros((bs, h_equi, w_equi), dtype=torch.float16, device=equi.device)
    for b in range(bs):
        uj = grid[b, 0, :, :].flatten()  # height coordinates
        ui = grid[b, 1, :, :].flatten()  # width coordinates
        j0 = torch.floor(uj).long()
        j1 = torch.ceil(uj).long()
        i0 = torch.floor(ui).long()
        i1 = torch.ceil(ui).long()
        j0 = torch.clamp(j0, 0, h_equi - 1)
        j1 = torch.clamp(j1, 0, h_equi - 1)
        i0 = torch.clamp(i0, 0, w_equi - 1)
        i1 = torch.clamp(i1, 0, w_equi - 1)
        js = torch.cat([j0, j0, j1, j1])
        is_ = torch.cat([i0, i1, i0, i1])
        mask[b, js, is_] = 1.0
